...
- Если не хватает данных (фамилия и имя, группа, дата, количество занятий, тип), задайте уточняющий вопрос.
...
def handle_tool_call(name, args):
  ...
  if name == "update_student":
    ...
    if not student_id:
      return {"ok": False, "error": "Студент не найден. Укажите ID или фамилию и имя, а также группу."}, None
  ...
  if name == "record_absence":
    ...
    if not student_id and student_name:
      ...
      if len(matches) == 1:
        ...
        return {
          "ok": False,
          "error": "Найдено несколько студентов. Уточните группу или фамилию и имя.",
          "candidates": matches,
        }, None
      else:
